...
class BtcvaeLoss(BaseLoss):
    ...
    def __call__(self, data, recon_batch, latent_dist, is_train, storer,
        ...
        if storer is not None:
            ...
            # computed for storing and comparison purposes
...
def _reconstruction_loss(data, recon_data, distribution="bernoulli", storer=None):
    ...
    distribution : {"bernoulli", "gaussian", "laplace"}
        Distribution of the likelihood on each pixel. Implicitly defines the
        loss. Bernoulli corresponds to a binary cross entropy (bse) loss and is the
        most commonly used. It has the issue that it doesn't penalize the same
        way (0.1,0.2) and (0.4,0.5), which might not be optimal. Gaussian
        corresponds to MSE, and is sometimes used, but hard to train
        because it ends up focusing only a few pixels that are very wrong. Laplace
        corresponds to L1 solves partially the issue of MSE.
    ...
    if distribution == "bernoulli":
        ...
        loss = loss * 3  # empirical value to give similar values than bernoulli => use same hyperparameters
